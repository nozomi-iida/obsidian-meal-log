...
| 3.4 | 認証が有効なとき全経路が対象（メタデータを除く） | 遮断点 | `config.matcher`（manifest とアイコンのみ除外） | 認証フロー |
...
- **受入基準 3.4 との関係**: 3.4 は但し書きで「ホーム画面への追加に必要なメタデータ（マニフェストとアイコン）は対象から除く」と定めている。上記の除外はこの但し書きに対応する。**これ以上の除外は認めない**
...
- **適用漏れの防止**: `matcher` の除外は `/manifest.webmanifest` と `/favicon.ico` の 2 つに限る。どちらも中身のないメタデータで、vault への書き込み経路とは関係しない。**これ以上の除外を追加する変更は Revalidation Triggers に該当し**、影響の再確認を要する。この 2 つの除外は受入基準 3.4 の但し書きに対応する
